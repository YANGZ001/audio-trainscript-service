# Spec: xiaoyuzhou-episode-transcription

## Purpose

Defines the behaviour for transcribing Xiaoyuzhou podcast episodes. The system accepts a `xiaoyuzhoufm.com/episode/<id>` URL, fetches the public episode page to obtain metadata and the audio URL, downloads the M4A file (with caching), and transcribes it using the standard SSE event flow.

# Spec: bilibili-audio-cache

## Purpose

Defines caching behaviour for Bilibili audio files downloaded during transcription. Audio files are stored on disk keyed by `bvid` so that repeated requests for the same video skip the download step. The cache is backed by a named Docker volume to survive container restarts.

...
MODEL="${MODEL:-}"
URL="${1:-https://www.xiaoyuzhoufm.com/episode/69d7b5e4e2c8be3155ccc32b}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
EPISODE_ID=$(echo "$URL" | grep -oE '[0-9a-f]{24}' | head -1)
...
python3 - "$DONE_DATA" "$EXPECTED_DIR/${EPISODE_ID}_transcript.txt" <<'EOF'
import sys, json
try:
